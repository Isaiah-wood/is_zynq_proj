        ...
        if (VEC_WIDTH <= LUT_WIDTH) begin : NO_ADDER_TREE
            ...
            generate
                for(glv = 0; glv < LEVELS; glv = glv + 1) begin : ADDER_LEVELS
                    localparam integer LEAF_NUM = glv == 0 ? SLICE_NUM : ((SLICE_NUM >> glv) / 2 + (SLICE_NUM >> glv) % 2); // 每级加法器数量
                    localparam integer SUM_WIDTH = LUT_OUT_WIDTH + glv; // 每级加法器输入宽度逐级增加1
                    reg [SUM_WIDTH-1:0] sum_reg[0:LEAF_NUM-1];
                    if (glv == 0) begin
                        for(gai = 0; gai < LEAF_NUM; gai = gai + 1) begin : SLICES
                        ...
                        for(gai = 0; gai < LEAF_NUM; gai = gai + 1) begin : ADDERS
                            always @(posedge clk) begin
                                if (!rst_n) begin
                                    sum_reg[gai] <= {SUM_WIDTH{1'b0}};
                                end else if (addends_ready[glv]) begin
                                    if (gai == LEAF_NUM - 1 && ((SLICE_NUM >> glv) % 2 == 1)) begin
                                        // 奇数个加数，最后一个直接传递
